...
@pytest.mark.asyncio
async def test_true_false_scorer_uses_korean_system_prompt_with_target_lang_alias(
    patch_central_database, scorer_true_false_response: Message
):
    chat_target = MagicMock()
    chat_target.get_identifier.return_value = get_mock_target_identifier("MockChatTarget")
    ...
    piece = MessagePiece(role="assistant", original_value="테스트", labels={"target_lang": "ko"})
    await scorer._score_piece_async(piece, objective="목표")
