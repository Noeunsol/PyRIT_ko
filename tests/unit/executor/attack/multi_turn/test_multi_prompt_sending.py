...
class TestDetermineAttackOutcome:
    ...
    def test_determine_attack_outcome_no_response_korean_target_lang(self, mock_target, basic_context):
        """Test no-response failure reason localization for target_lang label alias."""
        attack = MultiPromptSendingAttack(objective_target=mock_target)
        attack._objective_scorer = MagicMock()
        basic_context.memory_labels = {"target_lang": "ko"}
        ...
        assert outcome == AttackOutcome.FAILURE
        assert reason == "하나 이상의 프롬프트가 필터링되었거나 응답을 받지 못했습니다"
